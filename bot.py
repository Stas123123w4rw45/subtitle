...
import asyncio
import re
from groq import Groq  # <-- ЗМІНА: Використовуємо Groq замість faster_whisper
...
def find_ffmpeg():
    for name in ("ffmpeg", "ffmpeg.exe"):
        try:
            p = subprocess.run([name, "-version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            if p.returncode == 0:
                return name
        except Exception:
            pass
    # Якщо ви на Windows і ffmpeg лежить поруч
    local_path = os.path.join(os.getcwd(), "ffmpeg", "bin", "ffmpeg.exe")
    if os.path.isfile(local_path):
        return local_path
    return "ffmpeg" # Для Linux серверів зазвичай просто 'ffmpeg'

def escape_for_subtitles_filter(path):
    p = os.path.abspath(path)
    # На Windows шляхи у фільтрах FFmpeg потребують особливого екранування
    if os.name == 'nt':
        p = p.replace("\\", "/").replace(":", "\\:")
    return p
...
def transcribe_with_groq(file_path):
    """
    Відправляє файл на Groq API і отримує текст + таймкоди слів.
    """
    if not GROQ_API_KEY or "ВСТАВТЕ" in GROQ_API_KEY:
        raise ValueError("Не вказано GROQ_API_KEY в налаштуваннях!")
    ...
    with open(file_path, "rb") as file:
        ...
        transcription = client.audio.transcriptions.create(
            ...
            model="whisper-large-v3", # Найкраща модель
            response_format="verbose_json", # Щоб отримати таймкоди
            timestamp_granularities=["word"], # Важливо: таймкоди для кожного слова
            language="uk" # Можна прибрати або змінити на авто
        )

    # Формуємо список слів у форматі, який розуміє наш старий код: (start, end, word)
    all_words = []
    full_text = transcription.text
    
    # Groq повертає слова в transcription.words (якщо response_format="verbose_json")
    if hasattr(transcription, 'words'):
        for w in transcription.words:
            all_words.append((float(w['start']), float(w['end']), w['word'].strip()))
    else:
        # Фоллбек, якщо раптом слів немає (рідко)
        log.warning("Groq не повернув таймкоди слів.")
        
    return full_text, all_words

# --- [ЛОГІКА СИНХРОНІЗАЦІЇ] (Без змін, бо формат даних ми зберегли) ---
...
def _parse_transcript_for_tokens(raw_text):
    ...
    while i < n:
        ...
        if ch.isspace():
            ...
            space_run = raw_text[i:j]
            if '  ' in space_run or '\n' in space_run:
                next_is_line_start = True
            i = j
            continue
        j = i
        while j < n and (not raw_text[j].isspace()):
            j += 1
        word = raw_text[i:j]
        if word:
            idx = len(tokens)
...
def _events_from_layout(tokens, token_times, manual_starts):
    n = len(tokens)
    if n == 0 or not token_times or len(token_times) != n:
        return []
    ...
    for lines in pages:
        ...
        if inds[0] - 1 >= 0:
            left_bound = max(left_bound, mids[inds[0] - 1])
            
        if inds[-1] < n - 1:
            right_bound = min(right_bound, mids[inds[-1]])

        min_sec = 0.9 
        if right_bound - left_bound < min_sec:
            right_bound = left_bound + min_sec
        ...
        for ln in lines:
            ...
            for j in ln:
                ...
                if clean_word: 
                    clean_words.append(clean_word.upper())
            
            if clean_words:
                parts.append(escape_ass_text(" ".join(clean_words)))
...
def process_video_with_edits(video_path, all_words_original, edited_text, crf, style_settings):
    ...
    log.info("Крок 3/3: Впалювання субтитрів (FFmpeg)...")
    ff = find_ffmpeg()
    if not ff:
    ...
    # Додаємо перевірку, чи існує папка fonts, щоб не було помилок FFmpeg
    vf_filter_parts = [f"subtitles='{sub_escaped}'"]
    if os.path.exists(fontsdir_path):
        vf_filter_parts.append(f"fontsdir='{fontsdir_escaped}'")
    
    vf_filter = ":".join(vf_filter_parts)
    ...
    log.info(f"Обробку завершено: {out_path}")
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Крок 1: Отримання відео, витягування аудіо, транскрибація через Groq."""
    ...
    try:
        ...
        # Витягуємо аудіо з відео для прискорення Groq (легший файл)
        await message.reply_text("Витягую аудіо та розпізнаю текст... (Groq API) 🚀")
        
        ff = find_ffmpeg()
        audio_path = tmp_video_file_name + ".mp3"
        # Конвертуємо в легкий MP3
        ...
        try:
            ...
            await message.reply_text(f"Помилка Groq API: {e}")
            return ConversationHandler.END
        finally:
...
async def handle_audio_transcription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отримання аудіо, транскрибація через Groq (без відеоряду)."""
    message = update.message
    ...
    try:
        ...
        # Визначаємо ім'я (для voice message його може не бути)
        ...
        await message.reply_text("Розпізнаю через Groq API... 🚀")
...
async def handle_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Запускаємо обробку в окремому потоці, щоб не блокувати бота
        ...
        # Перевірка розміру для Telegram (50MB)
        ...
        if processed_file_size_mb > 49.0:
            ...
            await update.message.reply_text("Готово! Надсилаю... (Крок 3/3) 🚀")
...
def _get_style_menu(user_data):
    ...
    return text, InlineKeyboardMarkup([keyboard_size, keyboard_color, keyboard_font])
...
if __name__ == "__main__":
    ...
    # Обробник аудіо (ОКРЕМО від діалогу)
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            STATE_RECEIVE_EDIT: [
                ...
                CallbackQueryHandler(handle_style_buttons, pattern='^style_')
            ],
        },
        fallbacks=[
            CommandHandler('start', start_command), 
